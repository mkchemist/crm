...
/* harmony default export */ __webpack_exports__["default"] = ({
  mounted: function mounted() {
    ...
    this.getActiveCycle().then(function () {
      _this.getCycles().then(function () {
        _this.getReportInterval();
      });
    });
  },
  data: function data() {
  ...
  methods: {
    ...
    getReportInterval: function getReportInterval() {
      ...
      return _helpers_http_service__WEBPACK_IMPORTED_MODULE_1__["httpCall"].get('admin/v1/setting/report-interval').then(function (_ref3) {
        var data = _ref3.data;
        _this4.reportInterval = data.data;
        _this4.isReportIntervalFetched = true;
      })["catch"](function (err) {
        console.log(err);

        _this4.$toasted.error('Something went wrong');
      });
    },
    ...
    saveActiveCycle: function saveActiveCycle() {
      ...
      _helpers_http_service__WEBPACK_IMPORTED_MODULE_1__["httpCall"].post("admin/v1/setting/active-cycle", request).then(function (_ref4) {
        var data = _ref4.data;

        _this5.handleResponse(data, function (data) {
          _this5.getActiveCycle();
        });
      })["catch"](function (err) {
        console.log(err);
      });
     ...
     */
    saveReportInterval: function saveReportInterval() {
      var _this6 = this;

      var request = {
        interval: this.reportInterval
      };
      _helpers_http_service__WEBPACK_IMPORTED_MODULE_1__["httpCall"].post('admin/v1/setting/report-interval', request).then(function (_ref5) {
        var data = _ref5.data;

        _this6.handleResponse(data, function (data) {
          _this6.getReportInterval();
        });
      })["catch"](function (err) {
        console.log(err);

        _this6.$toasted.error('Something went wrong');
      });
    }
  }
});
      ...
      _c("div", { staticClass: "p-2 border rounded" }, [
            ...
            [
              _vm.reportInterval
                ? _c("input", {
                    directives: [
                      {
                        name: "model",
                        rawName: "v-model",
                        value: _vm.reportInterval,
                        expression: "reportInterval"
                      }
                    ],
                    staticClass: "form-control form-control-sm",
                    attrs: {
                    ...
                    domProps: { value: _vm.reportInterval },
                    on: {
                      input: function($event) {
                        if ($event.target.composing) {
                          return
                        }
                        _vm.reportInterval = $event.target.value
                      }
                    }
                  })
                : _c("loader-component")
        ...
        _c("div", { staticClass: "p-2 text-right" }, [
          _c(
            ...
            {
              staticClass: "btn btn-sm btn-primary",
              on: { click: _vm.saveReportInterval }
            },
            [
              _c("span", { staticClass: "fa fa-save" }),
              _vm._v(" "),
