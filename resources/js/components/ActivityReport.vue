<template>
  <div class="p-2">
    <div class="p-2 row mx-auto">
      ...
      <div class="col-lg-3">
        <!-- user control -->
        <user-filter-box :users="users" :data="reportData" :on-filter="onFilterData" :on-reset="resetDataFilter" />
        <!-- end of user control -->
        <!-- Date control -->
        <date-filter-box :data="reportData" :on-filter="onFilterData" :on-reset="resetDataFilter"  :dateField="`start`" />
        <!-- End of Date control -->
      </div>
      ...
      <div class="col-lg mx-auto px-0 shadow pb-5">
        ...
        <div class="p-2">
          <div v-if="reportData.length">
            <table-component :data="reportData" :heads="tableHeaders" :notResponsive="true" :headClass="`bg-success text-light`" >
...
export default {
  ...
  methods: {
    getReports() {
      ...
      this.reports = [];
      httpCall.get('activity-reports', request)
      .then(({data}) => {
