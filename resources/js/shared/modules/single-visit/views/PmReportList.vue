...
export default {
  ...
  methods: {
    callPMCollectionService() {
      this.$store
        .dispatch("SingleVisitModule/PMCollection")
        .then(({ data }) => {
          data.data = this.formatPMCollectionResource(data.data);
          this.list = data.data;
          this.fetched = true;
        })
     ...
     * @return {Array}
     */
    formatPMCollectionResource(data) {
    ...
    editRowAction(id) {
      this.closeActionModal().then(() => {
        this.$router.push("/single-visit/pm/edit/" + id);
      });
    },
     deleteRowAction(id) {
      this.$swal({
        ...
        if(res.isConfirmed) {
          return httpCall.post("v1/single/customer/"+id, {_method: "DELETE"})
          .then(({data}) => {
            if(data.code) {
              this.$swal({
                title :"Deleted",
